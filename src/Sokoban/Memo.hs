...
memo2 ::
     (Hashable a, Hashable b, Eq a, Eq b, Show a, Show b) => MVar (H.HashMap (a, b) c) -> (a -> b -> c) -> a -> b -> c
memo2 cache = curry2 . memo cache . uncurry2

memo3 ::
     (Hashable a, Hashable b, Hashable c, Eq a, Eq b, Eq c, Show a, Show b, Show c)
  => MVar (H.HashMap (a, b, c) d)
  -> (a -> b -> c -> d) -> a -> b -> c -> d
